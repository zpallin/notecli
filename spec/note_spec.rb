...
RSpec.describe Note do
  ...
  describe Book do
    ...
    it 'can list its contents as names' do
      FakeFS do
        ...
        p1 = Page.create 'b1/p1'
        p2 = Page.create 'b1/p2'
        expect(b1.list_names).to eq(%w[p1 p2])
      end
      FakeFS.clear!
    end
    ...
    it 'can list its contents as pages' do
      FakeFS do
        ...
        p1 = Page.create 'b1/p1'
        p2 = Page.create 'b1/p2'
        expect(b1.list_pages.map(&:name)).to eq(%w[p1 p2])
      end
      FakeFS.clear!
    end
  ...
  describe Page do
    ...
    context 'Page::process_pages' do
      it 'takes page names as argument and filters them to pages array' do
        FakeFS do
          pages = Page.process_pages %w[f1 f2]
          expect(pages.first.class).to eq(Page)
          expect(pages.map(&:name)).to eq(%w[f1 f2])
        end
        FakeFS.clear!
      end
      ...
      it 'can take match as a value to run string matches instead' do
        FakeFS do
          pages = Page.process_pages %w[f1 f2], match: true
          expect(pages).to eq([])
  ...
  describe Config do
    ...
    it 'must be explicitly created or its directories will not appear' do
      FakeFS do
        config = Config.new
        expect(Dir.exist?(config.store_path)).to eq(false)

        config = Config.create
        expect(Dir.exist?(config.store_path)).to eq(true)
      end
      FakeFS.clear!
    end
  ...
  context 'Note::page_op' do
    ...
    it 'can also pass lambdas for :one, :many, and :none pages found' do
      FakeFS do
        ...
        end.to output("test in order: ([\"f1\", \"f2\"])\nmany\n").to_stdout
      end
    end
  end
  ...
  describe History do
    it 'adds a page to the history' do
      FakeFS do
        ...
        expect(history.list).to eq(%w[f1 f2])
      end
      FakeFS.clear!
    end
    ...
    it 'will try to create the ~/.notecli/history file if it does not exist' do
      FakeFS do
        expect(File.exist?('~/.notecli/history')).to eq(false)
        history = History.new
    ...
    it 'controls line numbers via config' do
      FakeFS do
        ...
        expect(history.list).to eq(%w[f1 f2 f3 f4 f5 f6 f7 f8 f9 f10])

        f11 = Page.create 'f11'
        expect(f11).to receive(:system)
        f11.open

        expect(history.list).to eq(%w[f2 f3 f4 f5 f6 f7 f8 f9 f10 f11])
      end
      FakeFS.clear!
    end
  end
